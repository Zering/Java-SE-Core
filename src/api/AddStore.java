	...
	public void addStoreIdByStoreName(String path, int sheetNum, int storeNameCol, int storeIdCol) {
		...
		try {
			...
			for (int i = 1; i <= map.size(); i++) {
				sql = "SELECT s.* from T_STORE s where s.DEPARTMENT_ID in (SELECT d.ID from T_DEPARTMENT d WHERE d.PARENT_ID = 109) AND s.STORE_NAME='"
...
//		String userpath = "C:/Users/luco/Desktop/外勤系统/数据调整/合作医疗-User.xlsx";
		opStore.searchCHR1004ByStoreId(path, 4, 19, 16);
